...
SELECT department_id,
       ...
       LAST_VALUE(salary) OVER (PARTITION BY department_id ORDER BY salary DESC 
                                ROWS BETWEEN 2 PRECEDING AND UNBOUNDED FOLLOWING) AS "Lowest_Salary_in_Dept_Upto_2nd_Row",
       LAST_VALUE(salary) OVER (PARTITION BY deparment_id ORDER BY salary DESC
                                ROWS BETWEEEN 2 PRECEDING AND CURRENT ROW) AS "Lowest_Salary_in_Dept_Upto_2nd_Row_Default"                                                                
FROM   employee;
...
/*==============================================================================
  Title   : "CREATE TABLE" With GENERATED Value
==============================================================================*/

-------------------------------------------------------------------------------
-- id NUMBER    GENERATED ALWAYS AS IDENTITY
-- 1) Oracle always generates the value for the ID column.
-- 2) You cannot insert a value into this column. Will throw error if you try to insert.
-------------------------------------------------------------------------------

CREATE TABLE t_always (
    id NUMBER   GENERATED ALWAYS AS IDENTITY,
    name VARCHAR2(20)
);
...
-- 1) Oracle generates only if you don’t provide a value
-- 2) But it also allows NULL values to be inserted.
-------------------------------------------------------------------------------
...
-- 1) It accepts any non NULL value 
-- 2) Dont accept NULL values.
-- 3) Generates values if no values provided
-------------------------------------------------------------------------------

CREATE TABLE t_on_null (
    id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,
    name VARCHAR2(20)
);
...
/*==============================================================================
  Title   : "NOCACHE" in SEQUENCE; Default :- CACHE
  -- 1) Oracle does NOT pre-allocate sequence numbers in memory
  -- 2) Every time a sequence number is requested by SEQ.NEXT_VAL
  -- 3) Much fewer gaps (almost sequential)
  -- 4) It reduces gaps from crushes but fails in Rollback scenarios.
==============================================================================*/
