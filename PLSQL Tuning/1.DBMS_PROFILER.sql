...
/*
    ...
    2. Execute the code with multiple log statements and analyze which statement is taking more time
    3. Tune the SQL
    4. Tune the PLSQL code
*/

-- Steps to Start with

-- (1) ==> Create the Profiler Tables (from table creation script already present in Oracle Home directory) - proftab.sql
...
-- (2) ==> Clear the tables if any data is already present before
truncate table plsql_profiler_data;
delete from plsql_profiler_units;
delete from plsql_profiler_runs;

-- (3) ==> Invoke the DBMS_PROFILER.START_PROFILER
BEGIN
  DBMS_PROFILER.START_PROFILER('My Test Run');

  my_procedure;  -- Your PL/SQL code

  DBMS_PROFILER.STOP_PROFILER;
END;


-- (4) ==> Query to find which line took the most time and how many times it occured to run
